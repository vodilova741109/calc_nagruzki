...
function getDateNw(inputs, areaElement,totalConsumptionElement, label){       
    ...
    // }
        for (let i = 0; i < inputs.length; i++) {
            ...
            if ((inputs[i].value !== undefined || inputs[i].value !== null) && inputs[i].value === "" && inputs[i].type !="hidden"){
            ...
            // пропускать значение NaN и hidden, и передать переменные в массивы с числами и строками отдельно
            if(!isNaN(inputs[i].value) && inputs[i].type !="hidden"){
